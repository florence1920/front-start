	...
	<div class="wrapAll client">
		<c:if test="${sessionScope.memId==null}">
			<script>
				...
				location.href='/portfolio/mem/loginForm.pf';
			</script>
		</c:if>
		...
		<c:if test="${sessionScope.memId eq 'admin'}">
			<button onclick="window.location='/portfolio/admin/adminGroupDel.pf?gNo=${content.gNo}'" style="color:red">그룹삭제</button>
		</c:if>
			<div class="titWrap">
		...
		<div class="btnWrap">			
			<c:if test="${sessionScope.memId!=content.id && content.status==0 && memStatus==-1}">
				...
				<a href="/portfolio/travel/makingReq.pf?gNo=${content.gNo}"><p class="btn btnY">신청</p></a>
				<a href="/portfolio/travel/makingList.pf?pageNum=${pageNum}"><p class="btn btnC">뒤로</p></a>
				</c:if>						
			...
			<c:if test="${sessionScope.memId!=content.id && memStatus==1}">
				...
				<a href="/portfolio/travel/groupSpace.pf?gNo=${content.gNo}"><p class="btn btnY">그룹 공간으로</p></a>
			</c:if>
			...
			<c:if test="${memId eq 'admin'}"> 
				<a href="/portfolio/admin/adminGroup.pf"><p class="btn btnY">리스트</p></a>
			</c:if>
			<c:if test="${memId ne 'admin'}"> 
				<a href="/portfolio/travel/makingList.pf?pageNum=${pageNum}"><p class="btn btnY">리스트</p></a>
			</c:if>
