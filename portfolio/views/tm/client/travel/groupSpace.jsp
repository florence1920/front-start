...
<jsp:include page="/WEB-INF/views/include/header.jsp" />
   ...
   <div id="transcroller-body" class="wrapAll client">
      ...
      <c:if test="${idStatus!=1}">
         <script>
            ...
            location.href="/portfolio/travel/makingList.pf";
         </script>
      </c:if>
      ...
      <div class="gsWrap">
         ...
         <ul class="gsMenu">
            <li class="on"><a href="groupSpace.pf?gNo=${gNo}">그룹홈</a></li>
            <li><a href="gallery.pf?gNo=${gNo}">갤러리</a></li>
         </ul>
            ...
            <ul class="list">
               <li>
                  ...
                  <div class="inner">
                     ...
                     <p class="txt">
                        ${mem.id}님
                        <c:if test="${mem.id!=id}">
                           <input type="button" onclick="window.location='/portfolio/msg/messageWrite.pf?receiver=${mem.id}&sender=${id}'" value="쪽지쓰기"/>
                        </c:if>
                     </p>
                     </c:if>         
                     </c:forEach>
               ...
               <li>
                  ...
                  <div class="inner">                     
                     ...
                     <form action="scheduleModi.pf?gNo=${gNo}&sNo=${list.sNo}&sDate=${list.sDate}&sCont=${list.sCont}" method="get">
                     <div class="writeDown">
                        ...
                        <input type="button" value="삭제" class="btn" onclick="window.location='/portfolio/travel/scheduleDel.pf?gNo=${gNo}&sNo=${list.sNo}'" />
                        </c:if>
                     </div>
                     </form>
                     </c:forEach>                     
                        ...
                        <c:if test="${grpSpace.status==3}">
                           <c:if test="${memId == grpSpace.id}">
                              <c:if test="${empty grpSpace.genReview}">
                                  ...
                                  <form action="/portfolio/travel/genReviewPro.pf" id="cont2" name="reply" onsubmit="return check()" method="get">
                              ...
                              <c:if test="${!empty grpSpace.genReview}">
                                 ...
                                 <c:if test="${grpSpace.shared == 0}">   <!-- 그룹스페이스 쉐어드가 0이면 공개여부 보여줌 -->
                                    <form action="/portfolio/travel/groupSpace.pf" name="openOrbOpen" method="get">
                                       <input type="hidden" name="gNo" value="${grpSpace.gNo}" />
               ...
               <c:if test="${memId != grpSpace.id}">
                  ...
                  <c:if test="${!empty grpSpace.genReview}">
                     ...
                     <c:if test="${result == 1 }">
                      ...
                      <form action="/portfolio/travel/genReplyPro.pf" id="cont2" name="reply" method="get">
   ...
   $(document).ready(function(){   
      $('.accepted').submit(function(event){
         ...
         $.ajax({
            url: "/portfolio/travel/accepted.pf",
            type: "POST",
            dataType: "json",
            cache: false,
      ...
      $('.rejected').submit(function(event){
         ...
         $.ajax({
            url: "/portfolio/travel/rejected.pf",
            type: "POST",
            dataType: "json",
            cache: false,
            contentType: "application/json",
      ...
      $('#chatForm').submit(function(event){
         ...
         $.ajax({
            url: "/portfolio/travel/sendChat.pf",
            type: "POST",
            dataType: "json",
            contentType: "application/json",
            data: JSON.stringify(data),
   ...
   function changeStatus(gNo, status){
      ...
      $.ajax({
         url: "/portfolio/travel/changeStatus.pf",
         type: "POST",
         dataType: "json",
         contentType: "application/json",
         data: JSON.stringify(data),
