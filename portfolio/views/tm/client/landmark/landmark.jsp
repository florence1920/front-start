...
<div id="transcroller-body" class="myPageWrap myPageWrap2">
	...
	<script>
		$(document).ready(function(){
			...
			$.ajax({
				type: "POST",
				url: "/portfolio/land/landmarkList.pf",
				dataType: "json",
							...
							success: function(res){
								...
								if(arr.includes(res.lNo+"")==true){	//arr 원소가 문자열로 들어있으므로 res.lNo도 문자형으로 형변환하여 비교
									$('.like').html("<p>좋아요 한 랜드마크입니다</p>&nbsp;"	
											...
											+"<input type=\"button\" class=\"comBtn\" onclick=\"window.location='/portfolio/my/myLand.pf'\" value=\"나의 랜드마크\"/>"
									);
									$('.likeBtn').hide();
								}else{
									$('.like').html("");
			...
			$('.likeBtn').submit(function(event){
				...
				console.log(data);
				$.ajax({
					url: "/portfolio/land/landmarkLiked.pf",
					type: "POST",
					dataType: "json",
					cache: false,
					contentType: "application/json",
